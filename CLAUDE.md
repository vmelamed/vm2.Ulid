...
- Replace the internal `ReadOnlyMemory<byte>` wrapping a `byte[16]` array with a value-based representation, such as `ulong low; ulong high` or `Vector128<byte>`, to improve performance and memory layout.
